...
public class AggregatorTaskRealLifeTest {
    private static final String METRIC_NAME = "my.totally.awesome.and.long.Metric.Name";
    ...
    public void createMocks() {
        ...
        // Levels and Strategy
        RetentionLevel level = new RetentionLevel( "3daT5m", 5 * 60, 3, 24 * 60 * 60, "4daT1h" );
        nextLevel = new RetentionLevel( "4daT1h", 60 * 60, 2, 4 * 24 * 60 * 60, null );
        //strategy = new RetentionStrategy( "3da1w", 2, Arrays.asList( level, nextLevel ), 0, 0, 0 );
        sourceTable = new RetentionTable( level, 16324 );
        nextTable = new RetentionTable( nextLevel, 4081 );
    ...
    @Test
    public void test() {
        new AggregatorTask<AggregatorEnvironment>( environment, sourceTable, Optional.of( nextLevel ), Duration.ZERO ).run();
